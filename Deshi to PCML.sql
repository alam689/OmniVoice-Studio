with cte as (
select CreatedAt,100000 zid,AdviceID,AdviceDate,
( select customercode from PCML192.PremierCementOnline.dbo.customer where customerid= o.customerid) customercode,
...
(select max(xid) from caoutlet where xrow=o.shopno) xid,(select max(xoutletname) from caoutlet where xrow=o.shopno) xoutletname,
...
where customercode in (select xcus from cacus where xbloodgrp in ('FARIDPUR','KUSHTIA','GAZIPUR','MYMENSINGH','NARAYANGONJ')))
 and ShopNo<>'Shop_No')
...
where customercode in (select xcus from cacus where xsimcardno<>'Dhaka')) 
...
select  * from PCML192.PremierCementOnline.dbo.orderinfo 
where orderid not in (select  replace(xtrnnum,'D','') from oporderapp)

select  * from PCML192.PremierCementOnline.dbo.payment 
where PaymentID not in (select replace(xtrnnum,'D','') from oppaymentapp )


select  * from PCML192.PremierCementOnline.dbo.advice 
where adviceid not in (select  replace(xtrnnum,'D','') from opadviceapp)